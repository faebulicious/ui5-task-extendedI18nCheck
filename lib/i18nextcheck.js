...
const { retrieveFiles, convertLangCodeToFlag } = require('./utils');
...
module.exports = async function ({
  ...
  if (
    ...
    (await parseManifestFiles(manifestFiles, usedSymbols, log, options)) === false ||
    (await parseI18nFiles(i18nFiles, allI18nSymbols, langSymbols, log, options)) === false
  ) {
    return;
  }
  ...
  let unusedExceptions;
  if (options.configuration && options.configuration.usedKeys && Array.isArray(options.configuration.usedKeys)) {
    unusedExceptions = new Set(options.configuration.usedKeys);
  }

  // Check for Unused Symbols
  for (const symbol of allI18nSymbols.keys()) {
    if (!usedSymbols.has(symbol)) {
      if (!unusedExceptions || !unusedExceptions.has(symbol)) {
